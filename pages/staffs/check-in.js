...
export default function Dashboard({ staffs, checkedInList }) {

  const [checkedIn, setCheckedIn] = useState(checkedInList?.checkIns || []);

  const checkInStaff = (sid) => {
    setCheckedIn([...checkedIn, { sid, time: new Date().getTime()}]);
    add_doc(
      "check-ins", 
      { checkIns: [...checkedIn, { sid, time: new Date().getTime()}] },
        ...
        <div className="w-4/5">
          ...
          <div className='py-8 px-12 space-y-3'>
            ...
            {staffs?.map(({ sid, passport, firstname, lastname, status }, index) => (
              (!checkedIn.map(({sid: s}) => s).includes(sid)) &&
...
export async function getServerSideProps(context){
  ...
  return {
    props: {
      ...
      checkedInList: await get_single_doc("check-ins", new Date().toLocaleDateString().replace(/\//ig,"-")) || null,
